...
#define DEBOUNCE                                                               \
  10 // debounce itme in ms. Minimun time required for a button to be
     // stabilized.
...
void setup() {
  ...
  // If you want your mouse application to work as soon as plug-in the USB,
  // remove this line.
  while (!Serial)
    ;

  // sensor.begin(10, 1600); // to set CPI (Count per Inch), pass it as the
  // second parameter
...
void check_buttons_state() {
  ...
  for (int i = 0; i < NUMBTN; i++) {
    ...
    if (!btn_state[i] &&
        btn_buffers[i] == 0xFE) // button pressed for the first time
    {
      Mouse.press(btn_keys[i]);
      btn_state[i] = true;
    } else if ((btn_state[i] &&
                btn_buffers[i] ==
                    0x01) // button released after stabilized press
                          // force release when consequent off state (for the
                          // DEBOUNCE time) is detected
